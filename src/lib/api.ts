...
api.interceptors.response.use((response) => response, async (error) => {
  ...
  refreshing ??= axios.post(`${process.env.NEXT_PUBLIC_API_URL}/auth/refresh/`, { refresh: localStorage.getItem("mountain_refresh") }).then(({ data }) => {
  ...
  catch (refreshError) { localStorage.clear(); window.location.href = "/login"; throw refreshError; }
});
